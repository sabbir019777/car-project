...
Frontend on Netlify showed error: `GET car-project-server-side.vercel.app/api/cars/top-browse net::ERR_CONNECTION_REFUSED`
...
8. **Save the Vercel URL** shown in the deployment (e.g., `car-project-server-side.vercel.app`)
...
```
Frontend code: fetch("car-project-server-side.vercel.app/api/cars")
              ↓ (hardcoded URL)
...
```
Netlify env var: VITE_API_BASE = car-project-server-side.vercel.app
              ↓ (set before build)
...
1. Test backend directly: Open your Vercel URL in browser
   - Example: `car-project-server-side.vercel.app`
...
export const API_BASE =
  import.meta.env.VITE_API_BASE || "car-project-server-side.vercel.app";
```
