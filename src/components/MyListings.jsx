...
const MyListings = () => {
  ...
  useEffect(() => {
    ...
    if (!currentUser) throw new Error("User not logged in. Please login first.");
  ...
  useEffect(() => {
   ...
   const fetchCars = async () => {
      ...
      try {
   const response = await fetch("http://localhost:3000/api/car/my-listings", {
          headers: {
  ...
  const confirmDelete = async () => {
   ...
   toast.error("User not logged in. Please login first.");
    ...
    try {
   const response = await fetch(`http://localhost:3000/api/cars/${deleteCandidate._id}`, {
       method: "DELETE",
     headers: {
          "Content-Type": "application/json",
     Authorization: `Bearer ${accessToken}`,
...
<div className="text-center mb-16">
  ...
  <div className="grid md:grid-cols-3 gap-8 mb-16">
    ...
    <div className="text-4xl font-bold text-yellow-400 mb-2">{total}</div>
   <div className="text-gray-400 uppercase tracking-wide">Total Listings</div>
    </div>
      <div className="bg-gray-800 bg-opacity-70 backdrop-blur-md border border-white/10 rounded-2xl p-8 text-center">
   ...
   <div
    ...
    alt={car.name}
      ...
      e.target.src = "https://placehold.co/400x250/222/FFF?text=No+Image";
      }}
        />
    ...
    <div className="p-6 relative">
       ...
       <h3 className="text-lg font-bold text-white">{car.name}</h3>
          ...
          <div className="text-yellow-400 font-bold text-lg">${car.price}/day</div>
        </div>
   ...
   <p className="text-gray-500 text-xs mb-4">
     ...
     <div className="md:col-span-3 text-center p-10 bg-gray-800 rounded-xl border border-white/10">
              <h2 className="text-2xl text-yellow-400 mb-2">No Active Listings</h2>
   ...
   <UpdateCarModal
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
      <div className="bg-gray-900/95 border-2 border-red-500/50 rounded-xl p-8 max-w-sm w-full relative shadow-2xl shadow-red-600/20 transition-all duration-300">
        <h2 className="text-2xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-red-400 to-pink-500 mb-4">  ⚠️ System Alert
      </h2>
    <p className="text-gray-300 mb-6 border-b border-gray-700/50 pb-4">
    Are you sure you want to permanently delete <strong>{deleteCandidate.name}</strong>? This action cannot be undone. </p>
